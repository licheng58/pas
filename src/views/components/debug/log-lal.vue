...
<template>
  <div class="log-lal">
    <div class="lal-conditions">
      <div class="flex-v">
        ...
        <div class="logDataBody">
          ...
          <textarea v-model="logContent"
                    @change="changeLogAnaOptions(logTestConstants.Body)" />
        </div>
        <div>
    ...
    <div class="log-analysis">
      ...
      <div class="log-ana-btn bg-blue"
           @click="logAnalysis">{{ isLoading ? $t('waitLoading') : $t('analysis') }}</div>
    </div>
    <div class="lal-resp">
      <div class="log-metrics">
        ...
        <ul>
          <li>
            <span v-for="item of logMetricsHeader"
                  :class="item.value"
                  :key="item.value">
              {{ $t(item.label) }}
            </span>
          ...
          <li class="no-data"
              v-show="!rocketLogLAL.logTestResp.metrics.length">{{ $t('noData') }}</li>
          <li v-for="metric in rocketLogLAL.logTestResp.metrics"
              :key="metric.name">
            <span v-for="(item, index) of logMetricsHeader"
                  :class="item.value"
                  :key="item.value + index">
              <b v-if="item.value === 'tags'">
                <a v-for="t of metric.tags"
                   :key="t.key">{{ `${t.key}=${t.value};` }} </a>
              </b>
      ...
      <div class="error-tips">
        <i v-for="err of errorsMessage"
           :key="err.message">{{ err.message }}</i>
      </div>
    </div>
  </div>
</template>
