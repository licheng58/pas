...
<template>
  <div class="rk-endpoint-dependency"
    ...
    <div class="endpoint-dependency-chart">
      <DependencySankey v-if="stateTopo.endpointDependency.nodes.length"
                        :data="stateTopo.endpointDependency"
                        @showMetrics="showEndpointMetrics" />
      <div v-else
           class="endpoint-dependency-empty">
        No Endpoint Dependency
      </div>
    </div>
    <div class="mb-5 header"
         v-if="stateTopo.selectedEndpointCall">
      <span class="topo-tool-btn"
            @click="handleSetEdit">
        <rk-icon class="lg rk-icon"
      ...
      <span class="topo-tool-btn"
        ...
        <input id="endpoint-tool-bar-file"
               type="file"
               name="file"
               title=""
               accept=".json"
        ...
        <label for="endpoint-tool-bar-file">
          <rk-icon class="lg import"
                   icon="folder_open" />
        </label>
      </span>
      <span class="topo-tool-btn"
            @click="exportTopoEndpointDependencyMetrics">
        <rk-icon class="lg"
                 icon="save_alt"
    ...
    <div v-if="stateTopo.selectedEndpointCall"
         class="endpoint-dependency-metrics scroll_bar_style">
      <DashboardItem v-for="(i, index) in topoEndpointDependencyMetrics || []"
                     :key="i.uuid"
      ...
      <div v-show="stateTopo.editDependencyMetrics"
