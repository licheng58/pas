  ...
  <div class="event-list flex-h">
    ...
    <rk-sidebox width="1000px"
      ...
      <div class="config-box">
        <div class="series-type"
             v-show="type === pageEventsType.DASHBOARD_EVENTS">
          <label class="title">{{ $t('eventSeries') }}</label>
          <RkSelect :mode="'multiple'"
        ...
        <div v-show="type === pageEventsType.DASHBOARD_EVENTS">
          ...
          <ul>
            <li class="header">
              <span class="check">
                <input type="checkbox"
                       v-model="checkAllServiceEvents"
                       @click="checkServiceEvents" />
              </span>
              <span v-for="(item, index) of eventsHeaders"
                    :class="item.class"
            ...
            <li v-for="event in rocketComps.serviceEvents"
                :key="event.uuid"
                @click="viewEventDetail(event)">
              <span class="check">
                <input type="checkbox"
                       :checked="!!event.checked"
                       @click="selectEvents(event)" />
              </span>
              <span v-for="(item, index) of eventsHeaders"
                    :class="item.class"
        ...
        <div v-show="type !== pageEventsType.TOPO_ENDPOINT_EVENTS">
          ...
          <ul>
            <li class="header">
              <span class="check">
                <input type="checkbox"
                       v-model="checkAllInstanceEvents"
            ...
            <li v-for="event in rocketComps.serviceInstanceEvents"
                :key="event.uuid"
                @click="viewEventDetail(event)">
              <span class="check">
                <input type="checkbox"
        ...
        <div v-show="type !== pageEventsType.TOPO_INSTANCE_EVENTS">
          ...
          <ul>
            <li class="header">
              <span class="check">
                <input type="checkbox"
                       v-model="checkAllEndpointEvents"
                       @click="checkEndpointEvents" />
              </span>
              <span v-for="(item, index) of eventsHeaders"
                    :class="item.class"
                    :key="item.class + index">{{
            ...
            <li v-for="event in rocketComps.endpointEvents"
                :key="event.uuid"
                @click="viewEventDetail(event)">
              <span class="check">
                <input type="checkbox"
    ...
    <rk-sidebox :width="'1000px'"
      ...
      <div class="event-detail">
        <div class="mb-10 rk-flex"
             v-for="(item, index) in eventsDetailHeaders"
             :key="index">
          <span>{{ $t(item.text) }}: </span>
          <span v-if="item.class === 'parameters'">
            <span v-for="(item, index) of currentEvent[item.class]"
