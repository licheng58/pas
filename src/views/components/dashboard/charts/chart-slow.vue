  ...
  <div style="overflow: auto;height: 100%;"
    ...
    <div class="rk-chart-slow clear">
      <div class="rk-chart-slow-i"
        ...
        <div class="mb-5 ell"
             v-tooltip:top.ellipsis="i.name || ''">
          <span class="calls sm mr-10">{{ i.value }}</span>
          <span class="cp link-hover"
                @click="handleLink(i)">{{ i.name + getTraceId(i) }}</span>
        </div>
        <RkProgress :precent="(i.value / maxValue) * 100"
                    color="#bf99f8" />
      </div>
    </div>
    <rk-modal :show.sync="showModal"
              :title="$t('modalTitle')">
      <div>
        {{ $t('selectRedirectPage').replace('%s', redirectData.name) }}
      </div>
      <div class="mt-15">
        <router-link :to="redirectData.log"
                     class="rk-chart-slow-link mr-20">{{ $t('log') }}</router-link>
        <router-link :to="redirectData.trace"
                     class="rk-chart-slow-link mr-20">{{ $t('trace') }}</router-link>
      </div>
    </rk-modal>
  </div>
</template>
