...
<template>
  <div class="rk-dashboard-comp">
    <nav class="rk-dashboard-comp-nav mb-15">
      <a class="rk-dashboard-comp-nav-i b mr-20"
         v-show="value.length"
         @click="
          current = key;
          configMode = false;
      ...
      <div class="pt-10 r">
        <a class="rk-btn ghost sm"
           @click="configMode = !configMode">{{ $t('templateConfig') }}</a>
      </div>
    </nav>
    <div class="rk-dashboard-comp-wrapper">
      <div v-if="configMode"
           class="mb-15">
        <textarea class="long mb-5 sm rk-dashboard-textarea"
                  rows="3"
                  v-model="config"></textarea>
        <a class="rk-btn ghost sm mr-10"
           @click="handleCopy(config)">{{ $t('copy') }}</a>
        <a class="rk-btn ghost sm mr-10"
           @click="handleReset">{{ $t('reset') }}</a>
        <a class="rk-btn ghost sm"
           @click="handleApply">{{ $t('apply') }}</a>
      </div>
      <div v-else
