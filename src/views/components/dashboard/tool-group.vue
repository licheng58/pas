...
<template>
  <nav class="rk-dashboard-group">
    <span v-for="(i, index) in rocketComps.tree || []"
      ...
      <svg v-if="rocketGlobal.edit && rocketComps.group !== index"
    ...
    <a class="rk-dashboard-group-add"
      ...
      <div class="rk-dashboard-group-add-box"
        ...
        <div class="sm grey  mb-5 mr-10"
             v-show="type !== DASHBOARDTYPE.METRIC">{{ $t('templateConfig') }}</div>
        <select v-model="templateName"
                class="rk-dashboard-group-sel"
                v-show="type !== DASHBOARDTYPE.METRIC">
        ...
        <input class="mb-5 rk-dashboard-group-input"
               type="text"
               v-model="name" />
        <a class="rk-btn r vm long tc confirm"
