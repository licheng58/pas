...
<template>
  <div>
    <div :class="['profile-item', 'level' + data.parentId]"
         :style="{ color: data.topDur ? '#448dfe' : '#3d444f' }">
      <div :class="['thread', 'level' + data.parentId]"
           :style="{ 'text-indent': data.parentId * 4 + 'px', width: `${thread}px` }">
        <svg class="icon vm cp trans"
    ...
    <div v-show="data.children && data.children.length && displayChildren"
