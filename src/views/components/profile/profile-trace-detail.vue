  ...
  <div class="profile-trace-dashboard flex-v">
    <div class="profile-trace-detail-wrapper mb-5 blue sm">
      <select class="profile-trace-detail-ids">
        <option v-for="i in currentSegment.traceIds"
                :value="i"
                :key="i">{{ i }}</option>
      </select>
      <select class="profile-trace-detail-ids"
              @change="spanModeChange($event)">
        <option value="include"
                key="include">{{ $t('includeChildren') }}</option>
        <option value="exclude"
                key="exclude">{{ $t('excludeChildren') }}</option>
      </select>
    ...
    <TraceDetailChartTable :data="segmentSpans"
