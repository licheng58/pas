...
<template>
  <div class="profile-task-list flex-v">
    <div class="profile-task-wrapper flex-v">
      ...
      <div class="rk-trace-t-wrapper">
        <div class="rk-no-data"
             v-show="!taskListSource.length">No Data</div>
        <table class="rk-trace-t">
          <tr class="rk-trace-tr cp"
              v-for="(i, index) in taskListSource"
              @click="selectTask(i)"
              :key="index">
            <td class="rk-trace-td"
                :class="{
              ...
              <div class="ell mb-5">
                ...
                <a class="profile-btn r"
                   @click="viewTask($event, i)"
                   v-tooltip:bottom="{ content: $t('taskView') }">
      ...
      <rk-sidebox :width="'50%'"
        ...
        <div class="rk-trace-detail">
          ...
          <div class="mb-10 clear">
            <span class="g-sm-4 grey">{{ $t('monitorTime') }}:</span><span class="g-sm-8 wba">{{ selectedTask.startTime | dateformat }}</span>
          </div>
          <div class="mb-10 clear">
            <span class="g-sm-4 grey">{{ $t('monitorDuration') }}:</span><span class="g-sm-8 wba">{{ selectedTask.duration }} min</span>
          </div>
          <div class="mb-10 clear">
          ...
          <h5 class="mb-10"
              v-show="selectedTask.logs && selectedTask.logs.length">{{ $t('logs') }}.</h5>
          <div class="log-item"
            ...
            <div v-for="(d, index) in instanceLogs[i]"
                 :key="index">
    ...
    <div class="profile-trace-wrapper profile-segment flex-v">
      ...
      <div class="rk-trace-t-wrapper">
        <div class="rk-no-data"
             v-show="!segmentList.length">No Data</div>
        <table class="rk-trace-t">
          <tr class="rk-trace-tr cp"
              v-for="(i, index) in segmentList"
              @click="selectTrace(i)"
              :key="index">
            <td class="rk-trace-td"
                :class="{
