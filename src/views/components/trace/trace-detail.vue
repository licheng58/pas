...
<template>
  <div class="rk-trace-detail flex-v">
    <rk-sidebox class="rk-log-box"
                :width="'100%'"
                :show.sync="showTraceLogs"
                :title="$t('relatedTraceLogs')">
      <RkPage :currentSize="pageSize"
              :currentPage="pageNum"
              @changePage="turnLogsPage"
              :total="rocketTrace.traceSpanLogsTotal" />
      <LogTable :tableData="rocketTrace.traceSpanLogs || []"
                :type="`service`"
                :noLink="true">
        <div class="log-tips"
             v-if="!rocketTrace.traceSpanLogs.length">{{ $t('noData') }}</div>
      </LogTable>
    </rk-sidebox>
    <div class="rk-trace-detail-wrapper clear"
         v-if="current.endpointNames">
      <h5 class="mb-5 mt-0">
        <rk-icon icon="clear"
      ...
      <div class="mb-5 blue sm">
        <select class="rk-trace-detail-ids"
                @change="changeTraceId(i)">
          <option v-for="i in current.traceIds"
                  :value="i"
                  :key="i">{{ i }}</option>
        </select>
    ...
    <TraceDetailChartTree v-if="displayMode == 'tree' && current.endpointNames"
                          :data="spans"
                          :traceId="current.traceIds[0]" />
    <TraceDetailChartTable v-if="displayMode == 'table' && current.endpointNames"
                           :data="spans"
                           :traceId="current.traceIds[0]"
                           :showBtnDetail="false" />
    <TraceDetailStatisticsTable v-if="displayMode == 'statistics' && current.endpointNames"
                                :data="spans"
                                :traceId="current.traceIds[0]"
                                :showBtnDetail="false"
                                HeaderType="statistics" />

    <div v-if="!current.endpointNames"
         class="flex-h container">
