  ...
  <div class="rk-trace-search">
    <div style="display:flex">
      ...
      <div class="flex-h trace-select">
        <CommonSelector :hasSearch="true" title="服务 :" :value="service" @input="chooseService" :data="rocketTrace.services" icon="package" />
        <CommonSelector :hasSearch="true" title="实例 :" v-model="instance" :data="rocketTrace.instances" icon="disk" />
        <CommonSelector :hasSearch="true" title="端点 :" :value="endpoint" @input="chooseEndpoint" @search="searchEndpoint" :data="rocketTrace.endpoints" icon="code" />
        <CommonSelector title="状态 :" :value="traceState" @input="chooseStatus" :data="[
            { label: 'All', key: 'ALL' },
      ...
      <a class="rk-trace-clear-btn r max-c flex-h" @click="status = !status">
        <span class="mr-5 vm max-c">{{ $t('more') }}</span>
        <svg class="icon trans vm" :style="`transform: rotate(${status ? 180 : 0}deg);`">
          <use xlink:href="#arrow-down"></use>
        </svg>
      </a>

      <a class="rk-trace-search-btn bg-blue r max-c flex-h" style="margin:0 10px" @click="getTraceList">
        <svg class="icon mr-5 vm">
          <use xlink:href="#search"></use>
        </svg>
        <span class="vm max-c">{{ $t('search') }}</span>
      </a>
      <a class="rk-trace-clear-btn r mr-10 max-c flex-h" @click="clearSearch">
        <svg class="icon mr-5 vm">
          <use xlink:href="#clear"></use>
        </svg>
        <span class="vm max-c">{{ $t('clear') }}</span>
      </a>
    </div>
      ...
      <div class="flex-h">
        ...
        <div class="mr-15">
          ...
          <div class="rk-trace-search-range dib">
            ...
            <span class="grey vm" style="color:#000">-</span>
